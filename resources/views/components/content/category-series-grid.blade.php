    ...
    @foreach ($items as $item)
        ...
        <a href="/category-series-{{ $item->id }}-{{ $category }}.htm" class="w2a-cat-series-card">
            <div class="w2a-cat-series-header">
                ...
                <span class="w2a-cat-series-count"><i class="fa fa-play-circle" aria-hidden="true"></i>
                    {{ number_format((int) $item->count) }} مادة</span>
            </div>
            <div class="w2a-cat-series-body">
                <h3 class="w2a-cat-series-title">{{ trim((string) $item->title) }}</h3>
                @if ($authorName !== '')
                    <span class="w2a-cat-series-author"><i class="fa fa-user-circle-o" aria-hidden="true"></i>
                        {{ $authorName }}</span>
                @endif
